...
npm install
```
this will install all the reqiured dependencies for the project.
...
npx expo
``` 
A QRcode and various other things will appear.
![terminal](assets\images\terminal.png)
...
## Structure
- /constants: this folder only contains the file /Colors.ts which contains colors mix that will be used to style our pages.
- - ![constants folder](assets\images\constants.png)
- /components
- - components/_tests_/: this forder contains everything related to tests
...
- As mentionned earlier, you can register a dream by filling a form and submitting it.
...
- With the description you provided, an api call will be made. We will use the Meaning Cloud api to seach for entities through the text you provided as a description of your dream. The api's response will be then added to the newDream object as the "apiResponse" attribute which is itself an object.
- Then, we will try to seach for an array called dreamFormDataArray inside our AsyncStorage. If it does not exist, then we create an array called formDataArray. When this array is created (if not already), the newDream object will be pushed inside our new formDataArray and then this array will be set as "dreamFormDataArray" inside AsyncStorage, as JSON.
- Your new dream is now inside AsyncStorage.
- Also, you will receive a notification whenever you successfully submit a dream. **/!\ Important: notifications only work on physcal device not on emulated device**
...
- this dialog allows you to enter a tag name that will be given to your dream. You can give a dream as much tags as you want, they will all be displayed at the bottom of the card.
- ![tags](assets\images\tags.png)
5. ### Delete a tag
- if you click on "delete a tag" (supprimer un tag) a new dialog shows up.
- ![deleta a tag](assets\images\deleteATag.png)
...
- As you can see, the informations about my dream also changed. To actually have the new informations showing up, **you need to reload the app**.

7. ### Search
- Here I added more (meaningless) dreams and gave them different tags. If I use the seachbar on top of the page, I can search for a dream by its tags.
- ![search](assets\images\search1.png)
- Ypu can also search by title and by date. If nothing is found, your search will only find a "no result".
...
- A notification will be sent everytime a dream has successfully been registered.
...
- Informations not updating when modifying the description of a dream => reload the application (press R on your terminal)
- Informations being all mixed up when adding a new dream => reload the application (press R on your terminal)
